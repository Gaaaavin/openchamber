...
import { currentBurst, estimateTokens, formatTps, measureGrowth, smoothRate, tokensPerSecond } from './tps-math';
...
describe('currentBurst', () => {
  test('keeps only the last fifteen seconds', () => {
    const samples = [
      { tokens: 100, at: at(0) },
      { tokens: 5, at: at(14_000) },
      { tokens: 5, at: at(14_500) },
    ];
    expect(currentBurst(samples, at(16_000)).map((sample) => sample.tokens)).toEqual([5, 5]);
  });
...
describe('smoothRate', () => {
  test('an undefined raw rate resets the smoothing', () => {
    expect(smoothRate(42, undefined, 1_000)).toBe(undefined);
  });

  test('the first reading passes through unchanged', () => {
    expect(smoothRate(undefined, 42, 1_000)).toBe(42);
  });

  test('one time constant moves about 63 percent of the way', () => {
    const smoothed = smoothRate(0, 100, 3_000);
    expect(Math.abs((smoothed ?? 0) - 63.212) < 0.001).toBe(true);
  });

  test('zero elapsed time leaves the previous rate unchanged', () => {
    expect(smoothRate(20, 100, 0)).toBe(20);
  });

  test('negative elapsed time is treated as zero', () => {
    expect(smoothRate(20, 100, -1_000)).toBe(20);
  });
});
...
describe('tokensPerSecond', () => {
  test('tokens over the span they arrived in', () => {
    ...
    expect(tokensPerSecond(samples, at(1_000))).toBe(20);
  });
  ...
  test('undefined once nothing has arrived for a while, rather than a stale rate', () => {
    ...
    expect(tokensPerSecond(samples, at(1_600))).toBe(undefined);
  });

  test('undefined when there is nothing at all', () => {
    expect(tokensPerSecond([], at(0))).toBe(undefined);
  });
