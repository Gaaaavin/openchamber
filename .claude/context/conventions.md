...
- **2026-09-08**: Every distinct directory sent to OpenCode (header or `?directory=`) is a ~1 GB instance that lives until `POST /instance/dispose`. Rule for fork code: never fire an OpenCode request for a directory the user has not opened (project roots and the selected session's directory only); never send an instance-route request without a directory (it lands on the server cwd); worktree discovery stays because the sidebar needs it for ownership and visibility, only the bootstrap is gated (patch 6). `/global/*` routes are outside the instance middleware and ignore the directory.
