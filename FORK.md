...
| 2 | Live TPS readout in the floating "working" chip above the composer (`StatusRow`), shown only while the assistant works. Port of JDScript/opencode `fork-tps` (itself a port of the MIT `opencode-tps` TUI plugin, commit `3ecf08a82` there): 5 s window, 1.5 s burst gap, UTF-8 bytes / 5 estimate, dash when nothing is generating. Fed from sync-store text growth, no per-delta re-render, 1 Hz tick | `packages/ui/src/components/chat/fork/StatusRowTps.tsx`, `fork/tps-math.ts` (+ tests), 2 `// FORK:` lines in `StatusRow.tsx`, keys `fork.tps.*` in `fork.i18n.ts` | DONE |
...
5. Patch 2 (TPS) is done; it lives in the `StatusRow` chip rather than
   `ComposerStatusBar` because that bar only renders when there are todos or
   pending changes, so a meter there would have toggled a 40 px row on every
   turn. Not done: a settings toggle to hide it, and any visual check on
